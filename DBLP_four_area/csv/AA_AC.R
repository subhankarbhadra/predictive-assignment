...
A = rbind(cbind(APA, AC), cbind(t(AC), matrix(0, nrow = n.c, ncol = n.c)))
write.csv(A, "~/csv/lab-A.csv")
ACA = matrix(0, nrow = n.a, ncol = n.a)
for (i in 1:(n.a-1)){
for (j in (i+1):n.a){
if (t(AC[i,])%*%AC[j,] > 0) {ACA[i,j] = 1; ACA[j,i] = 1}
}}
write.csv(ACA, "~/csv/lab-ACA.csv")
...
P1 = N_err/N1
write.csv(A1, "~/all-hom1-output.csv")
...
P2 = N_err/N1
write.csv(A2, "~/all-hom2-output.csv")